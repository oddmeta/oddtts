...
def openai_tts_api_get_models():
    print("测试获取所有语音模型")

    response = requests.get(f"{base_url}/models")
    model_list = response.json()["data"]

    print(f"成功获取 {model_list}")
    
    return model_list

def openai_tts_api_get_voices():
    print("测试获取所有语音选项")
...
if __name__ == "__main__":
    model_list = openai_tts_api_get_models()
    if model_list:
        model = model_list[0]["object"]
    voices = openai_tts_api_get_voices()
    if voices:
        voice = voices[0]["short_name"]
    # get_voice_details(voice)
    openai_tts_api_synthesize(voice, text)
